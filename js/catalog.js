...
function updateCartCount() {
  ...
  const { fab, label, labelCount, labelCopy } = getCartFabElements();
...
function addToCart(productId, qty = 1) {
  ...
  updateCartCount();
  renderCartDrawer();
  if(typeof updateInlineCartButtons === 'function') updateInlineCartButtons();
}
      ...
      card.innerHTML = `
        ...
        <div class="cart-item__info">
          ...
          <div class="cart-item__qty">
            <button type="button" class="cart-qty-btn" data-cart-change="-1" data-product-id="${escapeHTML(product.id)}" aria-label="Quitar una unidad de ${escapeHTML(product.title)}">-</button>
            <span>${qty}</span>
            <button type="button" class="cart-qty-btn" data-cart-change="1" data-product-id="${escapeHTML(product.id)}" aria-label="Agregar una unidad de ${escapeHTML(product.title)}">+</button>
          </div>
          <div class="cart-item__meta">Precio: ${escapeHTML(product.price)}</div>
        </div>
        <button type="button" class="cart-item__remove" data-cart-remove="${escapeHTML(product.id)}" aria-label="Eliminar ${escapeHTML(product.title)} del carrito">Eliminar</button>
      `;
      list.appendChild(card);
    });
    checkoutBtn.disabled = false;
    checkoutBtn.textContent = 'Solicitar carrito por WhatsApp';
  }
...
function buildCartWhatsAppMessage() {
  ...
  let totalValue = 0;
  let message = 'Hola Futunet, deseo solicitar los siguientes productos:';
  items.forEach((product) => {
    const qty = cartState.items[product.id]?.qty || 0;
    const unitValue = parsePriceToNumber(product.price);
    const lineTotal = unitValue * qty;
    totalValue += lineTotal;
    message += `\n- ${qty} x ${product.title} (${product.brand}) - ${product.price} cada uno - Total: ${formatCurrency(lineTotal)}`;
  });
  if (totalValue > 0) {
    message += `\nTotal cotización: ${formatCurrency(totalValue)}`;
  }
  message += '\nPor favor envíame disponibilidad y la cotización final.';
  return message;
}
...
function handleCartInteraction(event) {
  ...
  if (changeBtn) {
    ...
    const delta = Number(changeBtn.dataset.cartChange) || 0;
...
function renderInlineAddButtonHTML(productId, variant = 'default') {
  const qty = cartState.items[productId]?.qty || 0;
  ...
  return `
    <button class="inline-add-btn product-add-btn" type="button" data-product-id="${escapeHTML(productId)}">
...
function initCartUI() {
  ...
  document.addEventListener('click', (event) => {
    ...
    if (inlineAdd) {
      ...
      addToCart(inlineAdd.dataset.productId);
      return;
    }
    const inlineChange = event.target.closest('[data-inline-change]');
    if (inlineChange) {
      event.stopPropagation();
      changeCartQuantity(inlineChange.dataset.productId, Number(inlineChange.dataset.inlineChange));
      return;
    }
  });
  ...
  const drawer = document.getElementById('cart-drawer');
  if (drawer) {
    drawer.addEventListener('click', handleCartInteraction);
