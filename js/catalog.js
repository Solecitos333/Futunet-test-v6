...
// Memoization for normalized searches
const __searchMemo = new WeakMap();

function getNormalized(obj, key) {
  let cache = __searchMemo.get(obj);
  if (!cache) {
    cache = {};
    __searchMemo.set(obj, cache);
  }
...
function renderCompactMobileCatalogView() {
  ...
  if (state.searchQuery) {
    ...
    for (let i = 0; i < mockDatabase.length; i++) {
      ...
      if (score > 0) {
        resultsWithScores.push({ product, score });
      }
    }
    resultsWithScores.sort((a, b) => b.score - a.score);
    const results = new Array(resultsWithScores.length);
    for (let i = 0; i < resultsWithScores.length; i++) {
      results[i] = resultsWithScores[i].product;
    }
...
function renderUI() {
  ...
  if (state.searchQuery) {
    ...
    resultsWithScores.sort((a, b) => b.score - a.score);
    let results = new Array(resultsWithScores.length);
    for (let i = 0; i < resultsWithScores.length; i++) {
      results[i] = resultsWithScores[i].product;
    }
