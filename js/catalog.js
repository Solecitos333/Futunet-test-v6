...
/**
 * Obtiene o calcula y cachea la versión normalizada de una propiedad de un objeto
 * para evitar llamadas redundantes a normalizeSearch() en bucles grandes.
 */
function getNormalized(obj, key) {
  ...
  if (obj[cacheKey] === undefined) {
    Object.defineProperty(obj, cacheKey, {
      ...
      enumerable: false, // Prevents leaking into JSON or loops
      writable: true,
      configurable: true
    });
  }
...
function renderCompactMobileCatalogView() {
  ...
  if (state.searchQuery) {
    ...
    // Performance optimization: Imperative loop instead of chained .map().filter().sort().map()
