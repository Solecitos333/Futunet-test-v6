...
/**
 * Normaliza y cachea propiedades de objetos para búsqueda rápida.
 */
function getNormalized(obj, key) {
  const cacheKey = '_' + key + 'Normalized';
  if (obj[cacheKey] !== undefined) return obj[cacheKey];
  const normalized = normalizeSearch(obj[key]);
  Object.defineProperty(obj, cacheKey, { value: normalized, enumerable: false });
  return normalized;
}
