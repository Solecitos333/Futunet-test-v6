...
/**
 * Lazy-caches and returns normalized string properties using a WeakMap
 * tied to object identity to optimize repeated search iterations.
 */
...
function renderCompactMobileCatalogView() {
  ...
  if (state.searchQuery) {
    ...
    for (let i = 0; i < mockDatabase.length; i++) {
      ...
      if (score > 0) {
        resultPairs.push({ product: p, score });
      }
    }
    const results = resultPairs
