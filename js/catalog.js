...
function renderCompactMobileCatalogView() {
  ...
  if (state.searchQuery) {
    ...
    // ⚡ Bolt: Optimize search results calculation by replacing chained .map().filter().sort().map()
    // with a single loop to score and filter, reducing intermediate array allocations.
    // Impact: Avoids allocating a full-length intermediate array of `{ product, score }` objects.
    let scoredResults = [];
