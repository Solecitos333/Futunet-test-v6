...
window.FutunetBilling = (function () {
  ...
  async function loadSettings() {
    ...
    if (doc.exists) {
      ...
      if (settings.name === 'Futunet Suministros SRL' || settings.name === 'Futunet') {
  ...
  function renderInvoicesTable() {
    ...
    paginated.forEach(inv => {
      ...
      if (inv.status !== 'cancelled') {
        ...
        actionsHtml += `
          <button class="table-btn table-btn-secondary" title="Editar" onclick="FutunetBilling.editQuote('${inv.id}')">
    ...
    tr.innerHTML = `
      <td>
        <div class="autocomplete-wrapper" style="position:relative; margin-bottom:4px;">
          <input type="text" class="form-input row-product-search" placeholder="Escribe para buscar..." oninput="FutunetBilling.searchRowProductAutocomplete(this, '${rowId}')" value="${itemData ? itemData.description : ''}" required autocomplete="off" />
      ...
      <td>
        <input type="number" class="form-input row-price" step="0.01" min="0" value="${itemData ? itemData.price : '0.00'}" required oninput="FutunetBilling.handleRowPriceQtyChange(this)" />
      </td>
      <td>
        <input type="number" class="form-input row-qty" min="1" value="${itemData ? itemData.qty : '1'}" required oninput="FutunetBilling.handleRowPriceQtyChange(this)" />
      </td>
      <td>
        <input type="number" class="form-input row-tax" step="0.01" min="0" value="${rowTaxAmount.toFixed(2)}" oninput="FutunetBilling.handleRowTaxChange(this)" data-override="${overrideStr}" />
      </td>
  ...
  async function saveInvoice(e) {
    ...
    if (editingInvoiceId) {
      ...
      await getDB().collection('futunet_invoices').doc(editingInvoiceId).update(invoiceData);
    } else {
      // Save new document to Firestore
      invoiceData.createdAt = firebase.firestore.FieldValue.serverTimestamp();
      await getDB().collection('futunet_invoices').add(invoiceData);
  ...
  async function viewInvoice(invoiceId) {
    ...
    // Set company name to Futunet Suministros
    const nameEl = document.getElementById('view-company-name');
    if (nameEl) {
